...
def team(request, team_id):
    ...
    invitiations = team.invitations.filter(status=Invitation.INVITED)

    return render(request, 'team/team.html', {'team': team, 'invitations': invitiations})
...
def invite(request):
    ...
    if request.method == 'POST':
        ...
        if email:
            invititations = Invitation.objects.filter(team=team, email=email)

            if not invititations:
                ...
                messages.info(request, 'The user was successfully sent an invitiation e-mail.')
...
def plans(request):
    ...
    if request.GET.get('cancel_plan', ''):
        try:
            ...
            error = 'Something went wrong during cancellation. Please try again.'
